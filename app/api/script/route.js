...
export async function POST(request) {
  ...
  if (!profile || profile.credits < 1) {
    return NextResponse.json(
      { error: "Keine Videos mehr verfügbar. Upgrade deinen Plan." },
      { status: 402 }
    );
  }
  ...
  if (!process.env.OPENAI_API_KEY) {
    ...
    if (!response.ok) {
      return NextResponse.json(
        { error: "Die KI konnte das Script gerade nicht erzeugen." },
        { status: 502 }
      );
    }
    ...
    output = {
      title: `AI Short: ${topic}`,
      script: data.output_text || "Kein Script zurückgegeben.",
      demo: false
    };
  }
  ...
  if (consumed !== true) {
    return NextResponse.json(
      { error: "Dein Kontingent wurde bereits aufgebraucht." },
      { status: 402 }
    );
  }
